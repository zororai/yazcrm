...
const props = defineProps({ categories: Array, isManager: Boolean });
...
function submit() {
    if (editing.value) {
        form.put(`/asset-categories/${editing.value.id}`, { onSuccess: () => { showForm.value = false; } });
    } else {
        form.post('/asset-categories', { onSuccess: () => { showForm.value = false; } });
    }
}

function remove(c) {
    if (!confirm(`Delete category "${c.name}"?`)) return;
    form.delete(`/asset-categories/${c.id}`);
}
</script>

<template>
    <AppLayout>
        <template #title>Asset Categories</template>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    <tr v-for="c in categories" :key="c.id" class="hover:bg-gray-50">
                        ...
                        <td class="table-td">{{ c.fixed_assets_count }}</td>
