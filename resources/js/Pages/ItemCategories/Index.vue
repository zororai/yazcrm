...
function submit() {
    if (editing.value) {
        form.put(`/item-categories/${editing.value.id}`, { onSuccess: () => { showForm.value = false; } });
    } else {
        form.post('/item-categories', { onSuccess: () => { showForm.value = false; } });
    }
}

function remove(c) {
    if (!confirm(`Delete category "${c.name}"?`)) return;
    form.delete(`/item-categories/${c.id}`);
}
</script>

<template>
    <AppLayout>
        <template #title>Item Categories</template>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    <tr v-for="c in categories" :key="c.id" class="hover:bg-gray-50">
                        ...
                        <td class="table-td">{{ c.items_count }}</td>
