...
<div class="app">
  ...
  <main class="main">
    ...
    <div class="page-hdr">
      <div>
        ...
        <div class="page-sub">
          ...
          {{ $lastUpdated ? \Carbon\Carbon::parse($lastUpdated)->format('d M Y, H:i') : 'N/A' }}
        </div>
      </div>
      <div class="hdr-right">
        <div class="live-pill"><span class="live-dot"></span> Live</div>
        <div class="total-pill">
          <div class="tv">{{ number_format($total) }}</div>
      ...
      <div class="top-grid">
        ...
        <div class="stat-stack">
          <div class="stat-mini">
            ...
            <div>
              <div class="sm-val">{{ number_format($total) }}</div>
          ...
          <div class="stat-mini">
            ...
            <div>
              <div class="sm-val">{{ number_format($validTotal) }}</div>
          ...
          <div class="stat-mini">
            ...
            <div>
              <div class="sm-val">{{ number_format($repeatTotal) }}</div>
        ...
        <div class="glass">
          ...
          <div class="donut-wrap">
            ...
            <div class="donut-center">
              <div class="donut-pct">{{ $validPct }}%</div>
          ...
          <div class="ov-rows">
            <div class="ov-row">
              ...
              <span class="ov-val">{{ number_format($validTotal) }}</span>
              <span class="ov-chg up">+{{ $validPct }}%</span>
            </div>
            <div class="ov-row">
              <span class="ov-dot" style="background:#fbbf24"></span>
              <span class="ov-lbl">Repeat Callers</span>
              <span class="ov-val">{{ number_format($repeatTotal) }}</span>
              <span class="ov-chg neutral">{{ $repeatPct }}%</span>
            </div>
            <div class="ov-row">
              <span class="ov-dot" style="background:#f87171"></span>
              <span class="ov-lbl">Immediate Action</span>
              <span class="ov-val">{{ number_format($immediateAct) }}</span>
              <span class="ov-chg neutral">{{ $immPct }}%</span>
            </div>
          </div>
        </div>
      ...
      <div class="bottom-grid">
        ...
        <div class="glass">
          ...
          @forelse ($topPurpose3 as $i => $purp)
            ...
            @endphp
            <div class="ch-item">
              <div class="ch-ring {{ $rings[$i] }}">{{ $i === 1 ? '✓' : '○' }}</div>
              <div class="ch-body">
                <div class="ch-name">{{ $purp->purpose_of_call }}</div>
                <div class="ch-prog">{{ number_format($purp->cnt) }} / {{ number_format($total) }} interactions</div>
              </div>
          ...
          @if($byStatus->isNotEmpty())
            <div style="margin-top:16px">
              ...
              @foreach ($byStatus as $status => $cnt)
                ...
                <div class="pb">
                  <div class="pb-hdr">
                    <span class="pb-lbl">{{ ucfirst(str_replace('_',' ',$status)) }}</span>
                    <span class="pb-val">{{ number_format($cnt) }} ({{ $sPct }}%)</span>
                  </div>
                  <div class="pb-track">
                    <div class="pb-fill" style="width:{{ $sPct }}%;background:{{ $sc }}"></div>
                  </div>
                </div>
              @endforeach
            </div>
        ...
        <div class="glass">
          ...
          <div class="cal-grid">
            @foreach (['Su','Mo','Tu','We','Th','Fr','Sa'] as $dn)
              <div class="cal-dn">{{ $dn }}</div>
            @endforeach
            @for ($e = 0; $e < $calFirstDay; $e++)
              <div class="cal-d empty">0</div>
            @endfor
            @for ($d = 1; $d <= $calDays; $d++)
              <div class="cal-d {{ $d === $today ? 'today' : '' }}">{{ $d }}</div>
            @endfor
          </div>
        ...
        <div class="glass" style="position:relative;overflow:hidden">
          ...
          <div class="out-val">{{ number_format($immediateAct) }}</div>
    ...
    <div id="sec-demographics" class="section" style="display:none">
      ...
      <div class="s-card">
        ...
        @foreach ($byKeyPops as $kp)
          ...
          <div class="pb">
            <div class="pb-hdr"><span class="pb-lbl">{{ $kp->key_pops }}</span><span class="pb-val">{{ number_format($kp->cnt) }} ({{ $pct }}%)</span></div>
    ...
    <div id="sec-services" class="section" style="display:none">
      ...
      <div class="s-card">
        ...
        @foreach ($byService as $svc)
          ...
          <div class="pb">
            <div class="pb-hdr"><span class="pb-lbl">{{ $svc->services_requested }}</span><span class="pb-val">{{ number_format($svc->cnt) }} ({{ $pct }}%)</span></div>
    ...
    <div class="footer">
      Helpline Analytics &middot; Auto-refreshes every 2 minutes &middot; {{ now()->format('d M Y') }}
    </div>
...
const validPct   = {{ $validPct }};
const repeatPct  = {{ $repeatPct }};
const immPct     = {{ $immPct }};
