...
<div class="header">
  <div>
    ...
    <p class="sub">
      ...
      Last updated: {{ $lastUpdated ? \Carbon\Carbon::parse($lastUpdated)->format('d M Y H:i') : 'N/A' }}
    </p>
  </div>
...
<div id="tab-overview" class="tab active">
  ...
  <div class="kpi-row">
    <div class="kpi" style="border-left-color:var(--secondary)">
      <div class="icon">📞</div>
      <div class="value" style="color:var(--secondary)">{{ number_format($total) }}</div>
      <div class="label">Total Interactions</div>
      <div class="sub">All channels</div>
    </div>
    <div class="kpi" style="border-left-color:var(--success)">
      <div class="icon">✅</div>
      <div class="value" style="color:var(--success)">{{ number_format($validTotal) }}</div>
      <div class="label">Valid Interactions</div>
      <div class="sub">{{ $total ? round($validTotal/$total*100,1) : 0 }}% of total</div>
    </div>
    <div class="kpi" style="border-left-color:var(--accent)">
      <div class="icon">🔄</div>
      <div class="value" style="color:var(--accent)">{{ number_format($repeatTotal) }}</div>
      <div class="label">Repeat Callers</div>
      <div class="sub">{{ $total ? round($repeatTotal/$total*100,1) : 0 }}% repeat rate</div>
    </div>
    <div class="kpi" style="border-left-color:var(--teal)">
      <div class="icon">🤝</div>
      <div class="value" style="color:var(--teal)">{{ number_format($uptakeTotal) }}</div>
      <div class="label">Uptake Confirmed</div>
      <div class="sub">{{ $validTotal ? round($uptakeTotal/$validTotal*100,1) : 0 }}% of valid</div>
    </div>
    <div class="kpi" style="border-left-color:var(--danger)">
      <div class="icon">🚨</div>
      <div class="value" style="color:var(--danger)">{{ number_format($immediateAct) }}</div>
      <div class="label">Immediate Action</div>
      <div class="sub">{{ $total ? round($immediateAct/$total*100,1) : 0 }}% of total</div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="tbl-wrap">
      <table>
        ...
        <tbody>
          @php $rank = 0; $maxProv = $byProvince->first()?->cnt ?? 1; @endphp
          @foreach ($byProvince as $row)
          @php $rank++; $pct = $total ? round($row->cnt/$total*100,1) : 0; $bar = $maxProv ? round($row->cnt/$maxProv*100) : 0; @endphp
          <tr>
            <td>{{ $rank }}</td>
            ...
            <td style="min-width:120px">
              <div class="prog-track"><div class="prog-fill" style="width:{{ $bar }}%;background:var(--secondary)"></div></div>
            </td>
          </tr>
          @endforeach
...
<div id="tab-demographics" class="tab">
  ...
  <div class="card">
    ...
    @php $maxKp = $byKeyPops->first()?->cnt ?? 1; @endphp
    @foreach ($byKeyPops as $kp)
    @php $pct = $total ? round($kp->cnt/$total*100,1) : 0; $bar = $maxKp ? round($kp->cnt/$maxKp*100) : 0; @endphp
    <div class="prog-bar">
      <div class="prog-hdr">
        <span class="prog-lbl">{{ $kp->key_pops }}</span>
        <span class="prog-val">{{ number_format($kp->cnt) }} <span style="color:var(--muted);font-weight:400">({{ $pct }}%)</span></span>
      </div>
      <div class="prog-track"><div class="prog-fill" style="width:{{ $bar }}%;background:var(--purple)"></div></div>
    </div>
    @endforeach
    @if($byKeyPops->isEmpty())
...
<div id="tab-services" class="tab">
  ...
  <div class="card">
    ...
    @php $maxSvc = $byService->first()?->cnt ?? 1; @endphp
    @foreach ($byService as $svc)
    @php $pct = $total ? round($svc->cnt/$total*100,1) : 0; $bar = $maxSvc ? round($svc->cnt/$maxSvc*100) : 0; @endphp
    <div class="prog-bar">
      <div class="prog-hdr">
        <span class="prog-lbl">{{ $svc->services_requested }}</span>
        <span class="prog-val">{{ number_format($svc->cnt) }} <span style="color:var(--muted);font-weight:400">({{ $pct }}%)</span></span>
      </div>
      <div class="prog-track"><div class="prog-fill" style="width:{{ $bar }}%;background:var(--teal)"></div></div>
    </div>
    @endforeach
...
<div id="tab-calls" class="tab">
  ...
  <div class="kpi-row">
    <div class="kpi" style="border-left-color:var(--success)">
      <div class="icon">✅</div>
      <div class="value" style="color:var(--success)">{{ number_format($byValidity->get('valid', 0)) }}</div>
      <div class="label">Valid Calls</div>
    </div>
    <div class="kpi" style="border-left-color:var(--danger)">
      <div class="icon">❌</div>
      <div class="value" style="color:var(--danger)">{{ number_format($byValidity->get('invalid', 0)) }}</div>
  ...
  <div class="card">
    ...
    @php $maxPurp = $byPurpose->first()?->cnt ?? 1; @endphp
    @foreach ($byPurpose as $purp)
    @php $pct = $total ? round($purp->cnt/$total*100,1) : 0; $bar = $maxPurp ? round($purp->cnt/$maxPurp*100) : 0; @endphp
    <div class="prog-bar">
      <div class="prog-hdr">
        <span class="prog-lbl">{{ $purp->purpose_of_call }}</span>
        <span class="prog-val">{{ number_format($purp->cnt) }} <span style="color:var(--muted);font-weight:400">({{ $pct }}%)</span></span>
      </div>
      <div class="prog-track"><div class="prog-fill" style="width:{{ $bar }}%;background:var(--secondary)"></div></div>
    </div>
    @endforeach
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $pct }}%</td>
            <td style="min-width:120px"><div class="prog-track"><div class="prog-fill" style="width:{{ $bar }}%;background:var(--secondary)"></div></div></td>
          </tr>
          @endforeach
        </tbody>
      </table>
...
const byMode     = @json($byMode);
const byProvince = @json($byProvince);
const byGender   = @json($byGender);
const ageGroups  = @json($ageGroups);
const byMarital  = @json($byMarital);
const byService  = @json($byService);
const byReferral = @json($byReferral);
